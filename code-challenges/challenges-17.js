...
const recursionPattern = (int1, int2) => {
        var recursionPattern=[];
        function getPattern(x,y,recursionPattern){
          if (x>0 && recursionPattern.indexOf(x)==-1) {
            recursionPattern.push(x);
            return x-getPattern(x-y,y,recursionPattern)
          }
          if(x<0|| recursionPattern[0]>x)
          {
            recursionPattern.push(x);
            return x+getPattern(x+y,y,recursionPattern) 
          }
        ...
        getPattern(int1,int2,recursionPattern);
...
const filterLinks = (str) => {
        let rgx= /\w*(www.)\w*\W*(\.com|\.net|.org)\w*/;
        let filterLinks = str.match(rgx);
